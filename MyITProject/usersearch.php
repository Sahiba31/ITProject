<html>
	...
	<body>
		<?php
			$q = $_GET['q'];
			$con = mysqli_connect('localhost','root','','MyITProject');  //Connnecting with database
			if (!$con) 
			{
				die('Could not connect: ' . mysqli_error($con));
			}
			$sql="SELECT * FROM products_data WHERE title like '%".$q."%'";  //Used for matching and searching.
			$result = mysqli_query($con,$sql);  //Running query
			while($row = mysqli_fetch_array($result)) 
			{
				echo "<div id='img_div'>";   //Printing all the details
			...
			if(mysqli_num_rows($result) == 0) //If there is no match
			{
