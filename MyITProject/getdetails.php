...
<html>
	...
	<body>
		<?php
			$q = $_GET['q'];
			$con = mysqli_connect('localhost','root','','MyITProject'); //Connecting with database
			if (!$con) {
			...
			if($q == '*')
			{
				$sql = "SELECT * FROM Products_Data"; //Fetch all the data
			}
			else
			{	
				$sql="SELECT * FROM Products_Data WHERE Category = '".$q."'"; //Fetch the data of particular category
			}
			$result = mysqli_query($con,$sql);
			//For Admin only
			while($row = mysqli_fetch_array($result)) {
				echo "<div id='img_div'>";   //Display the details of products
